(function () {
  ...
  var noConflict = function (uri, options) {
    ...
    options = options || {};
    options.engineio = options.engineio || {};
    options.engineio.resource = options.engineio.resource || "engine.io";
    ...
    util = {
      addSocketEvents: function (socket) {
        var mark = "nowjs.";
        socket.on('message', function(message) {
          if (message.indexOf(mark) !== 0)
            return;
          var ev = message.substring(0, message.indexOf(":"));
          var data = message.substring(message.indexOf(":") + 1);
          if (data) {
            data = JSON.parse(data);
          }
          ev = ev.split('.');
          socket.emit(ev[1], data);
        });
        socket.sendJson = function(data) {
          var str = data.type + ":" + (data.data ? JSON.stringify(data.data) : "");
          socket.send(str);
        };
      },
      _events: {},
      // Event code from engine.io
      on: function (name, fn) {
        ...
        function setter(newVal) {
          if (val !== newVal && newVal !== fqnMap.get(fqn)) {
            ...
            if (val && typeof val === 'object') {
              ...
              socket.sendJson({type: 'nowjs.del', data: [fqn]});
              val = newVal;
            ...
            socket.sendJson({type: 'nowjs.rv', data: toReplace});
          }
          return newVal;
        }
    ...
    lib = {
      ...
      constructRemoteFunction: function (fqn) {
        var remoteFn = function () {
          ...
          socket.sendJson({type: 'nowjs.rfc', data: {fqn: fqn, args: args}});
        };
      ...
      processScope: function (obj, path) {
        ...
        for (var i in data) {
          if (util.hasProperty(data, i) && data[i] !== undefined) {
            socket.sendJson({type: 'nowjs.rv', data: data});
            break;
          }
        }
      },
        ...
        if (obj && typeof obj === 'object') {
          ...
          if (keys && typeof keys === 'object') {
            ...
            if (toDelete.length > 0) {
              socket.sendJson({type: 'nowjs.del', data: toDelete});
            }
          }
        }
    ...
    var dependencies = [
      { key: 'eio', path: '/' + now.core.options.engineio.resource + '/engine.io.js'}
    ];
    ...
    var scriptLoaded = function () {
      ...
      now.core.start = function() {
        socket = new eio.Socket(uri + '/', now.core.options.engineio || {});
        now.core.engineio = socket;
        util.addSocketEvents(socket);
        socket.on('open', function () {
          now.core.clientId = socket.id;
          lib.handleNewConnection(socket);
          // Begin intermittent scope traversal

          setTimeout(function () {
            lib.processNowScope();
            socket.sendJson({type: 'nowjs.rd'});
            if (++readied === 2) {
              nowReady = true;
              util.emit('ready');
            }
