...
exports.initialize = function (nowjs) {
  ...
  var User = function (socket) {
    ...
    this.user = {
      clientId: socket.id,
      cookie: nowUtil.parseCookie(socket.req.headers.cookie)
    };
  ...
  User.prototype.deleteVar = function (fqn) {
    ...
    this.socket.sendJson({type: 'nowjs.del', data: fqn});
  };
  ...
  User.prototype.set = function (fqn, val) {
    ...
    if (typeof val === 'object') {
      ...
      this.socket.sendJson({type: 'nowjs.rv', data: flattenedVal});
    } else {
      this.scopeTable.set(fqn, val);
      var toSend = {};
      toSend[fqn] = nowUtil.getValOrFqn(val, fqn);
      this.socket.sendJson({type: 'nowjs.rv', data: toSend});
    }
  };
