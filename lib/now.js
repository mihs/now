 ...
 */
Now.prototype.initialize = function (eio, server, options) {
  // Merge options
  ...
  this.server = eio.attach(server, this.options.engineio);

  // Need this to be separate from clientsMap.
  this.server.on('connection', function (socket) {
    var user = new self.User(socket);
    nowUtil.addSocketEvents(socket);
